...
contract PLCRVoting {
    ...
    */
    function commitVote(uint _pollID, bytes32 _secretHash, uint _numTokens, uint _prevPollID) external {
        ...
        uint prevPollID = (_prevPollID == _pollID) ? dllMap[msg.sender].getPrev(_pollID) : _prevPollID;

        require(validPosition(prevPollID, nextPollID, msg.sender, _numTokens));
        dllMap[msg.sender].insert(prevPollID, _pollID, nextPollID);
